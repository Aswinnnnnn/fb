...
user_profile = ImageTk.PhotoImage(Image.open("image1.jpg"))
...
for i in range(3):
    ...
    post_image_frame.pack(side=TOP)

    # Create a label for the profile picture //Actual comment
    profile_picture_label = Label(post_image_frame, image=user_profile, bg="white")
    profile_picture_label.pack(side=TOP, padx=1)
    ...
    post_caption_label = Label(post_caption_frame, text=post_captions[i], font=("Segoe UI", 12), fg="#333", bg="white", wraplength=600)
    ...
    post_image_label.pack(side=BOTTOM)


    # Create a frame for the like and comment buttons
    post_buttons_frame = Frame(post_caption_frame, bg="white")
    post_buttons_frame.pack(side=BOTTOM, fill=X)

    # Create a like button
    like_button = Button(post_buttons_frame, text="Like", font=("Segoe UI", 10), fg="#ffcccb", bg="white", bd=0)
    like_button.pack(side=LEFT)

    # Create a comment button
    comment_button = Button(post_buttons_frame, text="Comment", font=("Segoe UI", 10), fg="#19BDFF", bg="white", bd=0)
